...
void gameLoop(CHARACTER* ch[], int TurnNum, int NewDeadChar, int DeadChar, int AvoidChar)
{
	...
	while (++TurnNum)
	{
		NewDeadChar = 0; // 죽은 캐릭터 수 (이번 턴)
		AvoidChar = 0; // 공격 피한 캐릭터 수
		DeadChar = 0; // 죽은 캐릭터 수 (이번 턴 제외)
		int DeadCharIdxNum = 0; // 죽은 캐릭터의 Index 수

		for (int i = 0; i < MAX; i++)
		{
			if (ch[i] != NULL) // 살아있다면 : ch[i]가 NULL이 아닌 상태.
			{
